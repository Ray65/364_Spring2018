...
def readProjectsFile():
    ...
    for line in lines:
        ...
        circuitID, projectID = line.split()
        if projectID not in projectMap:
            projectMap[projectID] = [circuitID]
        else:
            projectMap[projectID].append(circuitID)
...
def readStudentsFile():
    ...
    for line in lines:
        ...
        name, ID = line.split("|")
        studentMap[name.strip()] = ID.strip()
        idMap[ID.strip()] = name.strip()

    # idMap = {v: k for k, v in studentMap.items()}
    return studentMap, idMap


def readCircuitFile(circuitID):

    filePath = "Circuits/circuit_{0}.txt".format(circuitID)
    ...
    circuits = [element.strip() for element in lines[4].split(",")]

    return students, circuits


def getParticipationByStudent(studentName):

    studentMap, _ = readStudentsFile()
    ID = studentMap[studentName]

    filePaths = glob("Circuits/*")

    validCircuits = []
    for filePath in filePaths:
        circuitID = filePath[-9:-4]
        students, _ = readCircuitFile(circuitID)

        if ID in students:
            validCircuits.append(circuitID)

    projectMap = readProjectsFile()

    # projects = {projectID for validCircuit in validCircuits
    #             for projectID, circuitList in projectMap.items()
    #             if validCircuit in circuitList}

    projects = set()
    for projectID, circuitList in projectMap.items():
        for validCircuit in validCircuits:
            if validCircuit in circuitList:
