...
def formatName(name):
    ...
    namePattern = r"(?P<part1>\w+)(?P<comma>,?)\s(?P<part2>\w+)"

    m = re.search(namePattern, name)

    if not m.group("comma"):
        result = "{0} {1}".format(m.group("part1"), m.group("part2"))
    else:
        result = "{0} {1}".format(m.group("part2"), m.group("part1"))
...
def getRejectedEntries():
    ...
    namePattern = r"^(\w+,?\s\w+)(\W+)$"

    rejectedNames = []

    for line in lines:
        m = re.search(namePattern, line)

        if m:
            name = formatName(m.group(1))
...
def getEmployeesWithIDs():
    ...
    namePattern = r"^(\w+,?\s\w+)"
    idPattern = r"(\{?[0-9a-f-]{32,36}\}?)"

    pattern = namePattern + separatorPattern + idPattern
...
def getEmployeesWithStates():
    ...
    separatorPattern = r".+\W{2,}"
    namePattern = r"^(?P<name>\w+,?\s\w+)"
    statePattern = r"(?P<state>\w+(\s\w+)?)\s$"

    "^(?P<name>\w+,?\s\w+).+\W{2,}(?P<state>\w+(\s\w+)?)\s$"
    pattern = namePattern + separatorPattern + statePattern
    ...
    for line in lines:
        ...
        if m:
            name = formatName(m.group("name"))
            state = m.group("state")
            targetNames[name] = state
